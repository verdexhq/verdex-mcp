...
async function testBrowserIssues() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "Snapshot preview:",
        snapshot1.text.substring(0, 200) + "...\n"
      );
    } catch (error: any) {
    ...
    try {
      const bridgeExists = await (bridge as any).page?.evaluate(() => {
        return typeof (window as any).__bridge !== "undefined";
      });
      console.log("Bridge exists in browser:", bridgeExists);

      if (bridgeExists) {
        const bridgeData = await (bridge as any).page?.evaluate(() => {
    ...
    try {
      ...
      const bridgeAfterNav = await (bridge as any).page?.evaluate(() => {
        return typeof (window as any).__bridge !== "undefined";
      });
      console.log("Bridge exists after navigation:", bridgeAfterNav, "\n");
    } catch (error: any) {
      console.error(
        "✗ Navigation persistence test failed:",
        error.message,
        "\n"
      );
    }
    ...
    try {
      await (bridge as any).page?.evaluate(() => {
      ...
      const manualSnapshot = await (bridge as any).page?.evaluate(() => {
        try {
          const bridge = (window as any).__bridge;
