...
export class BrowserBridge {
     ...
     */
    async get_descendants(ref, ancestorLevel) {
        if (!this.page)
            throw new Error("Not initialized");
        return await this.page.evaluate((ref, ancestorLevel, helpersScript) => {
            // Ensure helpers are available
            ...
            Array.from(ancestor.children).forEach((child) => {
                ...
                Array.from(child.children)
                    ...
                    const refForGrandchild = Array.from(bridge.elements.entries()).find(([_, info]) => info.element === grandchild)?.[0];
                    if (refForGrandchild) {
